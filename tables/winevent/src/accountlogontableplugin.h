...
namespace zeek {
/// \brief A virtual table plugin that presents WEL successful account logon events (ID 4624)
class AccountLogonTablePlugin final : public IVirtualTable {
  struct PrivateData;
...
public:
  ...
  /// \brief Destructor
  virtual ~AccountLogonTablePlugin() override;
...
protected:
  ...
  /// \param configuration An initialized configuration object
  /// \param logger An initialized logger object
  AccountLogonTablePlugin(IZeekConfiguration &configuration,
                         IZeekLogger &logger);
