...
namespace zeek {
/// \brief A virtual table plugin that presents WEL registry value modification events (ID 4657)
class RegValModifiedTablePlugin final : public IVirtualTable {
  struct PrivateData;
...
public:
  ...
  /// \brief Destructor
  virtual ~RegValModifiedTablePlugin() override;
...
protected:
  ...
  /// \param configuration An initialized configuration object
  /// \param logger An initialized logger object
  RegValModifiedTablePlugin(IZeekConfiguration &configuration,
                          IZeekLogger &logger);
